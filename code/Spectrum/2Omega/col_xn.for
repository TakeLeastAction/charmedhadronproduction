...
110   continue
      ...
      do 03 k=1,25    ! 角度 101 central 
      do 01 i=1,50    ! 初始化
      param(i)=0.0
 01   continue
      param(1)= pt_Omega(circle)   !(j-1)*0.875/4.0  !(j-1)/1.     !           ! 横动量
      param(2)=3.1415926/12.0*(k-1) ! 角度
      param(3)=3            ! 粒子数
      param(4)=qms !1.115 !
      param(5)=qms !1.115 !
      param(6)=param(5)
      ...
      param(9)=param(5)
      n=param(3)
      call vegas(coles,0.0001,n*7-3,800000,100,0,0,param,yout,
     c yout_err ,yout_sd)
      open(40,file="Omega.txt")  !Lam_H5_pm2_10_15
      write(40,*)yout
      open(41,file="Omega_err.txt")
      !Lam_H5_pm2_10_15
      write(41,*)yout_err
      open(42,file="Omega_sd.txt") 
       !Lam_H5_pm2_10_15
      write(42,*)yout_sd
 03   continue
 02   continue
...
831   continue    
 ...
 836    continue
     ...
     c *(rel_rmax(1,3)-rel_rmin(1,3))*acu_q*acu_p*acu_tao
      ...
      coles=qmTot*cosh(y)*acu_f*Bf*S*fw2*yidu_s**N 
      s=yidu**N
      if (N_yta.eq.1)then
      coles=0.0
      end if   
